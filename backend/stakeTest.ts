...
async function main() {
  ...
  const { destination, lamports } = (transferInst as any).parsed.info;

  if (destination !== destinationPubkey.toBase58()) return markFailed(dbTx.id);

  if (BigInt(lamports) !== dbTx.amount) return markFailed(dbTx.id);
