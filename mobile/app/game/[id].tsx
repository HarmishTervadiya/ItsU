    ...
    return (
        <SafeAreaView className="flex-1 bg-background">
            <View className="flex-1 relative overflow-hidden rounded-[32px] border-4 border-[#12121A] m-2 bg-panel">
                ...
                <View className="absolute top-16 left-0 right-0 h-[45%] z-10" pointerEvents="none">
...
const PlayerCharacter = ({ player, position, isDead }: { player: any, position: any, isDead: boolean }) => {
    ...
    const opacityAnim = React.useRef(new Animated.Value(isDead ? 0.4 : 1)).current;
    ...
    useEffect(() => {
        if (isDead && !wasDead.current) {
            ...
            Animated.sequence([
                ...
                Animated.parallel([
                    Animated.spring(skullScaleAnim, { toValue: 1, friction: 5, useNativeDriver: true }),
                    Animated.timing(skullOpacityAnim, { toValue: 1, duration: 200, useNativeDriver: true })
                ]).start();
            });
    ...
    return (
        ...
        >
            <View className={`mb-1 px-2 py-0.5 rounded-md border border-white/10 ${isDead ? 'bg-red-900/80' : 'bg-[#12121A]/80'}`}>
                <Text className={`text-[9px] font-black tracking-wider uppercase ${isDead ? 'text-red-300 line-through' : 'text-white'}`}>{player.name}</Text>
            </View>
