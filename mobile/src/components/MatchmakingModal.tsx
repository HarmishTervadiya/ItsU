...
import { accentHex, primaryHex } from "../constants/color";
...
export const MatchmakingModal = ({
  ...
  const { lobbySocket, connectToLobby } = gameStore();
  ...
  const pendingStakeRequestRef = useRef<AbortController | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      pendingStakeRequestRef.current = null;
    }
  }, [isOpen]);

  const handleClose = useCallback(() => {
    // If we are still creating the DB stake intent, abort before wallet launch.
    if (step === "select" && loading) {
      cancelRequestedRef.current = true;
      pendingStakeRequestRef.current?.abort();
      pendingStakeRequestRef.current = null;
      setLoading(false);
    }
  ...
  const handleEnterQueue = async () => {
    ...
    try {
      ...
      const stakeAmount =
        currency === "SOL" ? STAKE_AMOUNT_SOL : STAKE_AMOUNT_SKR;
      const lamports =
        currency === "SOL" ? STAKE_AMOUNT_LAMPORTS : STAKE_AMOUNT_SKR_RAW;

      // Step 2: Record intent in DB
      console.log("[MatchmakingModal] Recording stake intent...");
      const abortController = new AbortController();
      pendingStakeRequestRef.current = abortController;

      const { success: recordSuccess, error: recordError } =
        await addStakeTransactionApi(
          {
      ...
      if (cancelRequestedRef.current) {
        console.log("[MatchmakingModal] Stake request canceled before wallet");
        return;
      }
      ...
      try {
        ...
        // Wallet hook already maps and displays wallet-specific errors.
        console.error("[MatchmakingModal] Wallet send failed:", walletError);
        return;
      }
      ...
      console.log(
        "[MatchmakingModal] Joining queue with signature:",
        signature,
      );
      const { success } = await joinQueueApi(signature);
  ...
  return (
    ...
    >
      <GestureHandlerRootView style={{ flex: 1 }}>
        <Pressable
          className="flex-1 justify-center items-center bg-[#0a0a10]/80 p-4"
          onPress={handleClose}
        >
          {/* Modal Container */}
          <Pressable
          ...
          >
            ...
            {step === "select" ? (
              <View key="select" className="gap-5 relative z-10 pt-2">
                ...
                <View className="flex-row gap-3">
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                    className={`flex-1 p-4 rounded-2xl border-4 items-center justify-center transition-all ${
                      currency === "SOL"
                        ? "bg-[#323552]"
              ...
              <View
                key="finding"
                className="items-center gap-6 py-6 pb-2 relative z-10"
              >
