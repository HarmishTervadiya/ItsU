        ...
        <View className="absolute inset-0 z-40" pointerEvents="box-none">
            ...
            {myPlayer?.role === 'WOLF' && (
                <View
                    ...
                    pointerEvents={(myPlayer?.isAlive && !game?.nightKillId) ? "auto" : "none"}
                >
                            ...
                            {alivePlayers.filter(p => !p.isMe).map(p => {
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        className={`border-2 rounded-xl px-4 py-2.5 flex-col items-center gap-1 shadow-lg ${isTargeted
                                    ...
                                    >
                                        ...
                                        <Text className={`text-[10px] font-black uppercase ${isTargeted ? 'text-white' : 'text-slate-300'}`}>
                                            {p.name}
                                        </Text>
                                    </TouchableOpacity>
                                );
