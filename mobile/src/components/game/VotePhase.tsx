...
export const VotePhase: React.FC<VotePhaseProps> = ({ alivePlayers, players }) => {
    const { game, submitVote } = useGameStore();
            ...
            <View className="mx-3 mb-3" pointerEvents="auto">
                ...
                >
                    {hasVoted ? (
                        ...
                        <>
                            <Text className="text-white font-black text-center uppercase text-xs tracking-widest mb-3">Who is the Wolf?</Text>
                                ...
                                <View className="flex-row flex-wrap justify-between gap-y-2">
                                    {alivePlayers.filter(p => !p.isMe).map(p => (
                                        ...
                                        >
                                            ...
                                            <Text className="text-xs font-black text-white flex-shrink" numberOfLines={1}>{p.name}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                            </ScrollView>
                            ...
                            >
                                <Text className="text-sm font-black text-accent uppercase tracking-widest text-center">🎲 Random Vote</Text>
                            </TouchableOpacity>
                        </>
                    )}
                </View>
